...
func CreatePlugin(registry *osdn.Registry, etcdClient *etcd.Client, hostname string, selfIP string, ready chan struct{}) (api.OsdnPlugin, oskserver.FilteringEndpointsConfigHandler, error) {
	fmp := &flannelmtPlugin{
	...
	if err := fmp.BaseInit(registry, fmp.flowController, fmp, hostname, selfIP, ready); err != nil {
		return nil, nil, err
	}
