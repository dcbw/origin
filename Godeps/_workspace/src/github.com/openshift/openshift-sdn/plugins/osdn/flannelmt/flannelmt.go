...
func NetworkPluginName() string {
	return "redhat/openshift-sdn-flannelmt"
}
...
func Master(registry *osdn.OsdnRegistryInterface, clusterNetworkCIDR string, clusterBitsPerSubnet uint, serviceNetworkCIDR string) {
	...
	kc, err := ovssubnet.NewFlannelMtController(registry, host, "", nil)
	if err != nil {
		glog.Fatalf("SDN initialization failed: %v", err)
	}
...
func Node(registry *osdn.OsdnRegistryInterface, hostname string, publicIP string, ready chan struct{}, plugin knetwork.NetworkPlugin, mtu uint) {
	mp, ok := plugin.(*FlannelMtPlugin)
	if !ok {
		glog.Fatalf("Failed to type cast provided plugin to a flannelmt type plugin")
	}
	kc, err := ovssubnet.NewFlannelMtController(registry, hostname, publicIP, ready)
	if err != nil {
		glog.Fatalf("SDN initialization failed: %v", err)
	}
	mp.OvsController = kc
